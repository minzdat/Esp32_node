...
io_conf.pin_bit_mask = (1ULL << ext_wakeup_pin_1) | (1ULL << ext_wakeup_pin_2);
io_conf.pull_down_en = 0;
io_conf.pull_up_en = 0;
ESP_ERROR_CHECK(gpio_config(&io_conf));

// Set default output values to low (0V)
gpio_set_level(ext_wakeup_pin_1, 0); // Set pin to low
gpio_set_level(ext_wakeup_pin_2, 0); // Set pin to low

printf("GPIO%d level: %d\n", ext_wakeup_pin_1, gpio_get_level(ext_wakeup_pin_1));
printf("GPIO%d level: %d\n", ext_wakeup_pin_2, gpio_get_level(ext_wakeup_pin_2));
