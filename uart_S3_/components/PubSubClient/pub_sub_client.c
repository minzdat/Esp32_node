...
esp_mqtt_client_handle_t g_mqtt_client2;  
...
void mqtt_init2(char *broker_uri, char *username ,char *client_id)
{
    g_mqtt_event_group = xEventGroupCreate();
    ...
    g_mqtt_client2 = esp_mqtt_client_init(&mqtt_cfg2); 

     // Initialize the MQTT client
    esp_mqtt_client_register_event(g_mqtt_client2, ESP_EVENT_ANY_ID, mqtt_event_handler, NULL);  // Register the event handler
...
void data_to_mqtt(char *data, char *topic, int delay_time_ms, int qos)
{
    ...
    if (len > 0) 
    {
        ...
        // esp_mqtt_client_publish(g_mqtt_client2, topic, json_data, strlen(json_data), qos, 0);
...
void data_to_mqtt2(char *data, char *topic, int delay_time_ms, int qos)
{
    xEventGroupWaitBits(g_mqtt_event_group,g_constant_ConnectBit,false,true,portMAX_DELAY);
    int len = strlen(data);
    if (len > 0) 
    {
        ...
        char *json_data = cJSON_Print(json_obj);
        // esp_mqtt_client_publish(g_mqtt_client2, topic, json_data, strlen(json_data), qos, 0);
        int ret=esp_mqtt_client_publish(g_mqtt_client2, topic, json_data, strlen(json_data), qos, 0);
        if (ret == -1)
        {
            ESP_LOGE(MQTT_TAG, "Failed to publish data!");
            // cJSON_Delete(json_obj);
...
void subcribe_to_topic(char *topic,int qos)
{
    ...
    xEventGroupWaitBits(g_mqtt_event_group,g_constant_ConnectBit,false,true,portMAX_DELAY);
    if(esp_mqtt_client_subscribe(g_mqtt_client2,topic,qos) == -1) 
    {
        ESP_LOGE(MQTT_TAG,"Failed to subcribe to topic");
    }
}
