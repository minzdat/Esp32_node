...
void wait_for_wifi_connection() 
{
    ...
    // Repeat indefinitely until WiFi is successfully connected
    while (true) 
    {
        // Check WiFi status
        if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK && strlen((const char*)ap_info.ssid) > 0) 
        {
            ESP_LOGI(TAG, "WiFi connected successfully to SSID: %s", ap_info.ssid);
            break;
        } 
        else 
        {
...
void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        unsigned long udp_port = 5011; // 5006 - 5013
